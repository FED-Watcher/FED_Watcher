...
def run_functional_verification(tokenizer, model, device):
    ...
    test_cases = [
        ...
        {
            "text": "Downward market volatility remains a significant concern for investors.",
            "expected": "negative",
        },
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["verify", "batch", "evaluate"],
        help="Operating mode: 'verify', 'batch', or 'evaluate'.",
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        help="Path to the directory with cleaned text files for batch mode.",
    )
    parser.add_argument(
        "--output_file", type=str, help="Path to save the output CSV file in batch mode."
    )
