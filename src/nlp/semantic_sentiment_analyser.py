...
def process_speech_directory(input_dir, chunk_output_file, doc_output_file):
    ...
    try:
        ...
        if not speech_files:
             print(f"Error: No speech files ending with '_powell_speeches.csv' found in '{input_dir}'.")
             return
    ...
    for filename in tqdm(speech_files, desc="Processing Files"):
        ...
        if date_chunk_scores:
            ...
            chunk_labels = [res['sentiment_label'] for res in all_chunk_results if res['meeting_date'] == meeting_date]
...
def main():
    parser = argparse.ArgumentParser(description="Process Jerome Powell's speeches using semantic chunking and FinBERT for sentiment analysis.")
    parser.add_argument('--input_dir', type=str, required=True, help="Path to the input directory containing speech CSV files.")
    parser.add_argument('--chunk_output', type=str, default='sentiment_results/semantic_chunk_sentiments.csv', help="Path to save the chunk-level output CSV.")
    parser.add_argument('--doc_output', type=str, default='sentiment_results/semantic_document_sentiments.csv', help="Path to save the document-level output CSV.")
