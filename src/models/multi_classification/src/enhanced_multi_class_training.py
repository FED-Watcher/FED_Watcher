...
original_features = ['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 
                     'Volume_ratio_vs_5days', 'Announcement',
                     'hawkish_dovish_ratio', 'net_sentiment_score',
                     'negative_proportion', 'neutral_proportion', 
                     'positive_proportion', 'sentiment_label']
...
df_announcements['market_return'] = (
    (df_announcements['Next_Close'] - df_announcements['Close']) / 
    df_announcements['Close'] * 100
)
...
df_announcements['market_magnitude_class'] = df_announcements['market_return'].apply(classify_return)
...
ax.set_title('Feature Correlation Matrix (Enhanced Feature Set)', 
...
class_labels = ['Strong Drop/n(-2)', 'Modest Drop/n(-1)', 'Neutral/n(0)', 
                'Modest Rise/n(1)', 'Strong Rise/n(2)']

disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_labels)
disp.plot(cmap='Blues', ax=ax, values_format='d')
ax.set_title('Confusion Matrix - Enhanced Multi-Class Model', 
             fontsize=14, fontweight='bold')
...
colors = ['steelblue' if cat == 'Sentiment' else 'darkgreen' if cat == 'Macro' else 'coral' 
          for cat in importance_df_sorted['Category']]
bars = ax1.barh(importance_df_sorted['Feature'], importance_df_sorted['Importance'], 
...
ax2.barh(category_importance.index, category_importance.values, 
         color=['coral', 'darkgreen', 'steelblue'], alpha=0.8)
...
predictions_df['correct_prediction'] = (predictions_df['actual_class'] == predictions_df['predicted_class'])
...
try:
    ...
    if improvement > 0:
        ...
        print(f"  Absolute Gain:            +{(accuracy - baseline_accuracy)*100:.2f} percentage points")
    else:
